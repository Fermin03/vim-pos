...
export default function CajasPage() {
  ...
  return (
    <>
                        ...
                        <tr key={c.id} className="group cursor-pointer border-b border-line last:border-none hover:bg-hover" onClick={() => setModal({ caja: c })}>
                          ...
                          <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                            <span className="inline-flex gap-1 transition-opacity lg:opacity-0 lg:group-hover:opacity-100">
                              <button type="button" title="Generar credenciales del dispositivo" disabled={generando === c.id} onClick={() => generarDispositivo(c)} className="flex h-10 w-10 items-center justify-center rounded border border-transparent lg:h-8 lg:w-8 text-ink-3 transition hover:border-line-strong hover:bg-surface hover:text-ink disabled:opacity-40">
                              ...
                              <button type="button" title="Editar" onClick={() => setModal({ caja: c })} className="flex h-10 w-10 items-center justify-center rounded border border-transparent lg:h-8 lg:w-8 text-ink-3 transition hover:border-line-strong hover:bg-surface hover:text-ink">
                              ...
                              <button type="button" title="Eliminar" onClick={() => setBorrar(c)} className="flex h-10 w-10 items-center justify-center rounded border border-transparent lg:h-8 lg:w-8 text-ink-3 transition hover:border-[#E8C5C0] hover:text-danger">
      ...
      {creds && (
        <Modal open onClose={() => setCreds(null)} title="Credenciales del dispositivo" className="w-full max-w-[480px] rounded-lg border border-line bg-surface p-6 shadow-xl">
