<!-- fuente: https://apisandbox.facturama.mx/guias/api-multi/csds · capturado 2026-09-03 -->

# Carga de CSDs para API Multiemisor

Los Certificados de Sello Digital (**CSD**), son elementos que permiten la creación de CFDIs

Si quieres conocer más de los CSD visita nuestro blog [SELLOS DIGITALES CSD](https://facturama.mx/blog/que-significa/sellos-digitales-csd/)

	Cada RFC con el que se desee emitir CFDIs debe tener su propio CSD

		Notas:

		**a)** Los CSDs cargados mediante este método, son independientes de los cargados en la Plataforma Web de Facturama (los de la plataforma son para API Web)

		**b)** Para la API Multiemisor, la carga de CSDs se realiza exclusivamente por este medio
		**c)** En el ambiente de pruebas puedes empleas los certificados del SAT [Certificados de pruebas del SAT](https://apisandbox.facturama.mx/guias/conocimientos/sellos-digitales-pruebas)

#### URL para la petición

#### POST

```
https://apisandbox.facturama.mx/api-lite/csds
```

#### Datos requeridos

	Rfc:  Clave del Registro Federal de Contribuyentes

	Certificate:  Certificado en base64

	PrivateKey:  Llave privada en base64

	PrivateKeyPassword:  Contraseña de la llave privada

#### Agregar CSD

```
{
        Rfc: "EKU9003173C9",
        Certificate: "",
        PrivateKey: "",
        PrivateKeyPassword:"12345678a"
    }
```

## Eliminar CSD

#### URL de la petición

#### DELETE

```
https://apisandbox.facturama.mx/api-lite/csds/{rfc}
```

	Rfc:  Clave del Registro Federal de Contribuyentes

#### Eliminar CSD - Ejemplo

```
https://apisandbox.facturama.mx/api-lite/csds/EKU9003173C9
```

## Actualizar CSD

#### URL de la petición

#### PUT

```
https://apisandbox.facturama.mx/api-lite/csds/{rfc}
	Ejemplo: https://apisandbox.facturama.mx/api-lite/csds/EKU9003173C9
```

#### Datos requeridos (los mismos que en la carga de CSDs)

	Rfc:  Clave del Registro Federal de Contribuyentes

	Certificate:  Certificado en base64

	PrivateKey:  Llave privada en base64

	PrivateKeyPassword:  Contraseña de la llave privada

#### Actualizar CSD - Ejemplo

```
{
        Rfc: "EKU9003173C9",
        Certificate: "",
        PrivateKey: "",
        PrivateKeyPassword:"12345678a"
    }
```
