<!-- fuente: https://apisandbox.facturama.mx/guias/nominas/catalogos/paymentfrequencies · capturado 2026-09-03 -->
...
**https://apisandbox.facturama.mx/catalogs/paymentfrequencies**
...
```
payment_method = facturama.catalog.paymentfrequencies
```
...
```
$lstNameIds = $facturama->get('catalogs/paymentfrequencies');
```
...
```
Facturama.Catalogs.PaymentFrequencies(function (result) {
...
```
List paymentForm = facturama.Catalogs().PaymentFrequencies();
```
...
```
var paymentForm = facturama.Catalogs.PaymentFrequencies;
```
...
self.client._credentials = ('username', 'password')
    lst_payment_methods = self.client.PaymentFrequencies.query()
```
