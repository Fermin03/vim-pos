<!-- fuente: https://apisandbox.facturama.mx/guias/nominas/catalogos/perceptions · capturado 2026-09-03 -->
...
**https://apisandbox.facturama.mx/catalogs/perceptions**
...
```
payment_method = facturama.catalog.Perceptions
```
...
```
$lstNameIds = $facturama->get('catalogs/perceptions');
```
...
```
Facturama.Catalogs.Perceptions(function (result) {
...
```
List paymentForm = facturama.Catalogs().Perceptions();
```
...
```
var paymentForm = facturama.Catalogs.Perceptions;
```
...
self.client._credentials = ('username', 'password')
    lst_payment_methods = self.client.Perceptions.query()
```
