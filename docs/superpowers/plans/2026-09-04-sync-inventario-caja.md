...
```
Comparar el tamaño que reporta GitHub con el del `.exe` local; luego subir `dist/latest.json` al bucket `actualizaciones` con `PUT` y cabecera `apikey`/`Authorization` del service_role (el controlador pide la clave a el dueño; no se guarda). Confirmar el feed con `?v=<timestamp>` porque la caché de borde tarda minutos.
