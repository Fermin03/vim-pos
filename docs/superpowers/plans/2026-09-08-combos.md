...
- **Numeración de migraciones:** la spec dice **0110**. Comprobar con `supabase migration list --linked` antes de fijarla y otra vez antes de `db push`: un número ya presente en remoto hace que `db push` la **salte en silencio** (ya pasó tres veces).
- **Versión del escritorio:** `main` está en `0.4.64`; esta entrega publica **0.4.65**. Mirar `desktop/package.json` en `main` antes de subirla, no asumir.
...
|---|---|
| `supabase/migrations/0110_combos.sql` | `productos.es_combo`, `combo_grupos`, `combo_opciones`, columnas padre-hijo en `ticket_items`, `agregar_combo_a_ticket`, rechazo de combos en `agregar_item_a_ticket`, cascada en `cancelar_item_ticket`, vistas de ventas, pull, `catalogo_version()`, `configuracion_tenant.combo_upsell_activo` |
...
| `desktop/src/sync-pull.mjs` (+ `.test.mjs`), `desktop/package.json` | dos tablas más en el pull; 0.4.65 |
...
```
Expected: la última remota es `0109` (si no, renumerar `0110` en todo este plan y en la spec §4) y el escritorio está en `0.4.64`.
...
### Task 1: Migración 0110 §1 — catálogo de combos y RLS

**Files:**
- Create: `supabase/migrations/0110_combos.sql`
...
`supabase/migrations/0110_combos.sql`:
...
```bash
git add supabase/migrations/0110_combos.sql supabase/tests/0015_combos.test.sql
...
### Task 2: Migración 0110 §2 — padre e hijos en el ticket, RPC y cancelación

**Files:**
- Modify: `supabase/migrations/0110_combos.sql` (agregar la sección 2 al final)
...
Al final de `supabase/migrations/0110_combos.sql`:
...
```bash
git add supabase/migrations/0110_combos.sql supabase/scripts/smoke_combos.sql
...
### Task 3: Migración 0110 §3 — vistas de ventas, pull y versión de catálogo

**Files:**
- Modify: `supabase/migrations/0110_combos.sql` (agregar la sección 3 al final)
...
```bash
git add supabase/migrations/0110_combos.sql packages/db/src/database.types.ts
...
### Task 15: Escritorio — pull de las dos tablas y versión 0.4.65
...
`desktop/package.json`: `"version": "0.4.65"` (confirmar antes que `main` sigue en `0.4.64`).
...
```
Expected: PASS; `verify-migraciones` ve la 0110 en la lista.

- [ ] **Step 6: Commit**

```bash
git add desktop/src/sync-pull.mjs desktop/src/sync-pull.test.mjs desktop/package.json
git commit -m "feat(escritorio): la caja baja combo_grupos y combo_opciones; 0.4.65" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```
...
- Migración 0110: `es_combo`, `combo_grupos`, `combo_opciones`, padre/hijos en `ticket_items`, `agregar_combo_a_ticket`, cascada al cancelar, vistas de ventas, pull, `catalogo_version`.
...
- Escritorio 0.4.65: baja las dos tablas.
...
1. `supabase migration list --linked` → la última es 0109. `supabase db push` → aparece 0110 aplicada (si no aparece, el número chocó: **no** seguir).
...
4. Instalador: **pasar la lista "Antes de empaquetar" del `desktop/RUNBOOK.md`** (checkout con `desktop/bin/postgrest.exe`, `main` actualizado, versión 0.4.65). `cd desktop && npm run dist` en segundo plano (>10 min). Verificar la fecha del `.exe` y que pese ~156 MB con `resources/bin/postgrest.exe` dentro. Publicar desde `/versiones` del panel de plataforma.
5. Comprobar en la nube, para el tenant de Knock-Out, que `sync_pull_snapshot` devuelve las claves `combo_grupos` y `combo_opciones` (vacías está bien) y que la caja instalada sube a 0.4.65 sola.
