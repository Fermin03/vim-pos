...
- [4. Fase 0 — Creación del tenant (el dueño)](#4-fase-0--creación-del-tenant-fermín)
...
| **D84** | Onboarding es "self-service guiado": el cliente lo hace solo, con prompts y validaciones, no el dueño haciéndolo por él | §1.2 — filosofía |
...
|---|---|---|---|
| 0. Creación del tenant | Obligatoria | No (la hace el dueño) | Pre-requisito de todo |
...
Para que el dueño pueda asesorar al cliente al cerrar venta:
...
Esta lista es lo que el dueño envía al cliente como "tarea previa" antes de la cita de onboarding:
...
## 4. Fase 0 — Creación del tenant (el dueño)

**Quién ejecuta:** SUPER_ADMIN (el dueño o equipo VIM Marketing)
...
| Notas internas | text | (opcional) | "Piloto, contacto el dueño, sin contrato firmado todavía" |
...
SELECT crear_tenant_con_owner(
  ...
  p_notas_internas   => 'Piloto el dueño, mayo 2026'
);
```
...
Si tienes dudas durante el proceso, contacta a el dueño:
...
**Importante:** si el dueño no activa en 7 días, se le envía recordatorio. Si no activa en 30 días, el dueño recibe alerta y decide si suspender el tenant o contactar al cliente.
...
**Si elige la tercera:** se crea ticket de soporte interno para el dueño. El tenant continúa sin CFDI por ahora.
...
| Venta de prueba falla en cobrar | Bug raro de cálculo en producto con modificadores | Capturar stacktrace, mostrar mensaje genérico al usuario, alerta a el dueño |
