...
| **Plataforma** | VIM (el dueño / equipo) | `apps/platform` | **Fuera del RLS** (`service_role`) | TODAS las empresas: alta, baja, planes, folios, métricas |
...
Rol de plataforma `SUPER_ADMIN` (el dueño / equipo VIM). **No vive en `usuarios_acceso`** (ese es a nivel tenant); vive a nivel plataforma. Sus accesos se registran en `super_admin_accesos` (ver §9). **D99.**
